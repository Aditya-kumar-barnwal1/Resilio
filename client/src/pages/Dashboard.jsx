...
const Dashboard = () => {
  ...
  // 2. Fetch Data & Setup Real-time Listener
  useEffect(() => {
    // A. Fetch existing reports from MongoDB
    const fetchEmergencies = async () => {
      try {
        const { data } = await axios.get(`${ENDPOINT}/api/v1/emergencies`);
        setEmergencies(data);
        setLoading(false);
      } catch (error) {
    ...
    // B. Listen for REAL-TIME updates from Socket.io
    ...
    <div className="flex h-screen bg-slate-100 font-sans">
      ...
      <aside className="w-64 bg-[#0f172a] text-white flex flex-col shadow-xl">
        ...
        <div className="p-4 border-t border-slate-800">
           <div className="flex items-center gap-2 bg-slate-800/50 p-3 rounded-lg text-[10px] text-green-400 uppercase tracking-widest font-bold">
        ...
        <section className="flex-1 flex p-6 gap-6 overflow-hidden">
          ...
          <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden min-w-[350px]">
             ...
             <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {loading ? (
                  <p className="text-center text-slate-400 mt-10">Connecting to Satellite...</p>
                ) : emergencies.length === 0 ? (
                  <p className="text-center text-slate-400 mt-10">No Active Emergencies.</p>
                ) : (
